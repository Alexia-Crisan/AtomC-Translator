...
typedef enum
{
	...
	OP_LOAD_I,	 // takes an address from stack and puts back the int value from that address
	OP_LOAD_F,	 // takes an address from stack and puts back the double value from that address
	OP_STORE_I,	 // takes from stack an address and int value, stores value at address, leaves value on stack
...
// inserts a new instruction after the specified instruction
// returns the newly added instruction
Instr* insertInstr(Instr* before, int op);

// deletes all instructions after the given one
